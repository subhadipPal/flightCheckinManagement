...
export default function Bookings() {
  ...
  const currentFlightSeatCapacity =
    flights.filter(flight => flight.id === bookings[0]?.flightId)[0]?.seating_capacity

  const PassengerInfo = lazy(() => import('./passengerInfo'))

  return (
    <>
      <Typography variant="h6" style={{marginLeft: '20px'}}>Booking information</Typography>
